...
export default {
  ...
  methods: {
    ...
    fetchItems(current_page) {
      var sourceUrl = this.source + '?order_column=' + this.orderBy.column + '&order_direction=' + this.orderBy.direction + '&search_term=' + this.searchTerm.trim() + '&page=' + current_page;
      axios.get(sourceUrl).then((res) => {
        this.pagination = res.data.pagination;
      }).catch((err) => {
        this.showAlertError(err); //Mixin
      });
